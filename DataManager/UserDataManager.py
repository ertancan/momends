...
class UserDataManager(DataManagerModel):
    ...
    def get_data_for_keyword(self, keyword):
        if keyword in CoreAnimationData.SPECIAL_DATA_KEYWORDS:
            _index = CoreAnimationData.SPECIAL_DATA_KEYWORDS.index(keyword)
            if _index == 0:  # Current photo title, Returning the current data, frontend will use its title
                return self.getPreviousData(RawData.DATA_TYPE['Photo'])
