...
class CreateMomendForm(forms.Form):
    ...
    start_date = forms.DateTimeField(widget=widgets.DateInput(format='%d %b, %Y'), required=True,
        input_formats=['%d %b, %Y'], initial=(date.today()-relativedelta(months = +1)))
    finish_date = forms.DateTimeField(widget=widgets.DateInput(format='%d %b, %Y'), required=True,
